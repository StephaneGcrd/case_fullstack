...
async def test_stream_chat_yields_run_start_and_text_delta():
    ...
    agent = make_fake_agent([], final_text_chunks=["Hello"])
...
async def test_stream_chat_surfaces_thinking_status_and_answer_once():
    ...
    agent = make_fake_agent(
        handler_events, final_text_chunks=["Here is the answer."]
    )
    service = ChatService(session_store=session_store, artifact_store=artifact_store)
    ...
    # The final answer appears exactly once (single text source, no duplication).
    assert text.count("Here is the answer.") == 1
    assert "<thinking>" not in thinking and "<thinking>" not in text
    assert "</thinking>" not in thinking and "</thinking>" not in text
